...
def load_sparse_adj_data_with_contextnode(adj_pk_path, max_node_num, num_choice, args):
    ...
    if use_cache:
        with open(cache_path, 'rb') as f:
            ...
            #未经任何处理的邻接列表的原始长度，记录图中每个节点拥有的连接数量。节点id，节点类型，节点相关性分数，处理后的邻接列表长度，一个列表或数组描述途中节点之间的连接，边的类型
            #tensor（48705）tensor（9741，5，200）tensor（9741，5，200）tensor（9741，5，200，1）tensor（9741，5）list（9741）list（9741）19
            # 未经任何处理的邻接列表的原始长度，记录图中每个节点拥有的连接数量。节点id，节点类型，节点相关性分数，处理后的邻接列表长度，一个列表或数组描述途中节点之间的连接，边的类型
            # tensor（48705）tensor（9741，5，200）tensor（9741，5，200）tensor（9741，5，200，1）tensor（9741，5）list（9741）list（9741）19
            # （9741，5，200）指的是9741个问题，每个问题5个选项，每个子图最大有200个节点。其中第一个给句子节点留的
    else:
...
def load_bert_xlnet_roberta_input_tensors(statement_jsonl_path, model_type, model_name, max_seq_length):
    ...
    class InputFeatures(object):
        ...
        def __init__(self, example_id, choices_features, label):
            self.example_id = example_id #问题的id
            self.choices_features = [
                {
                    'input_ids': input_ids,  #供给模型的tokenid序列
                    'input_mask': input_mask, #用于区分真实tokens与填充符号的编码
                    'segment_ids': segment_ids, #用来区分不同序列的段落id
                    'output_mask': output_mask, #针对cls和sep等特殊令牌的掩码
                }
                for _, input_ids, input_mask, segment_ids, output_mask in choices_features
            ]
            self.label = label #与正确答案选项对应的数值标签
    ...
    def read_examples(input_file):
        with open(input_file, "r", encoding="utf-8") as f:
            ...
            for line in f.readlines():
                ...
                #将字母变成数字索引，如果答案是A，那么对应数字是0，如果答案是B，对应数字是1，依次类推
                contexts = json_dic["question"]["stem"] #这个是单纯的问题题干
                if "para" in json_dic:
                ...
                examples.append(
                    InputExample(
                        example_id=json_dic["id"], #问题id
                        contexts=[contexts] * len(json_dic["question"]["choices"]), #内容列表*选择个数5个列表
                        question="",
                        endings=[ending["text"] for ending in json_dic["question"]["choices"]], #列表中有5个答案选项文本如letter
                        label=label
                    ))
        return examples
    ...
    def convert_examples_to_features(examples, label_list, max_seq_length,
        ...
        for ex_index, example in enumerate(tqdm(examples)):
            ...
            label = label_map[example.label] #label是取出值作为label
            features.append(InputFeatures(example_id=example.example_id, choices_features=choices_features, label=label))
            #返回问题id，选项特征（5个，分别是tokens, input_ids, input_mask, segment_ids, output_mask，一个label(其实是排序id)）
    ...
    #tokenizer = tokenizer_class.from_pretrained('aristo-roberta/')
    ...
    with open(statement_path, 'r', encoding='utf-8') as fin:
        for line in fin:
            ...
            all_dict[qid] = {
                ...
                'question_concept': instance_dict['question']['question_concept']
            }
    return all_dict
