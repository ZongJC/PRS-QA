...
class QAGNN_Message_Passing(nn.Module):
    ...
    def calculate_RF(self, edge_index,edge_type, n_nodes, n_relations):
        #初始化RF矩阵
        RF = torch.zeros(n_nodes,n_relations,dtype=torch.float32)
        #遍历每条边，更新RF矩阵
        for scr,dst,rel in zip(edge_index[0],edge_index[1],edge_type):
            RF[scr.item(),rel.item()] += 1

        return RF

    def calculate_EF_IEF(self,edge_index,edge_type,n_relations, total_nodes):
        #初始化EF矩阵
        EF = torch.zeros(n_relations,dtype=torch.float32)
        ...
        for rel in edge_type.unique():
            ...
            #提取这些边连接的所有节点
            nodes_linked = torch.cat([edge_index[0][mask],edge_index[1][mask]])
            unique_nodes = nodes_linked.unique()
            EF[rel] = unique_nodes.size(0)

        IEF = torch.log(total_nodes / (1 + EF))
        return IEF

    def calculate_RF_IEF(RF,IEF):
        #计算RF-IEF矩阵
        RF_IEF = torch.matmul(RF,IEF.unsqueeze(1)).squeeze(1)
        return RF_IEF
