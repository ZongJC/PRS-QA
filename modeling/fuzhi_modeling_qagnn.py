...
class QAGNN_Message_Passing(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, input_size, hidden_size, output_size,sent_dim,
        ...
        self.promptembeds = PromptEmbedding(input_dim=sent_dim ,output_dim=input_size , num_heads=4)
        self.text2graph = Text2graph(h=4,d_model=hidden_size,len_q=200,len_k=100,d_k=hidden_size,d_v=hidden_size)
        #self.Conv1dBiLSTM = Conv1dBiLSTM(in_channels=hidden_size,cnn_kernel_num=128,cnn_window_size=3, word_dim=hidden_size)
        self.Fusenet = FusionModule(h=4,d_model=hidden_size,len_q=200,len_k=100,d_k=hidden_size,d_v=hidden_size)
        self.graph2text = Graph2text(h=4,d_model=hidden_size,len_q=100,len_k=200,d_k=hidden_size,d_v=hidden_size)
        #self.MaskedBiLSTM = MaskedBiLSTM(input_dim=hidden_size, hidden_dim=hidden_size//2, num_layers=1)
    ...
    def forward(self, H, A, edge_lengths, node_type, node_score, sent_vecs_r, cache_output=False, last_hidden_states=None, lm_mask=None, gnn_mask=None):
        """
        ...
        _X ,promptembeds = self.mp_helper(_X, edge_index, edge_type, _node_type, _node_feature_extra, sent_vecs_r, edge_lengths,
...
class QAGNN(nn.Module):
    ...
    def forward(self, sent_vecs, concept_ids, node_type_ids, node_scores, adj_lengths, adj, edge_lengths, emb_data=None, cache_output=False,last_hidden_states=None, attention_mask=None):
        """
        ...
        gnn_output, promptembeds = self.gnn(gnn_input, adj, edge_lengths, node_type_ids, node_scores, sent_vecs_r,last_hidden_states=last_hidden_states, lm_mask=attention_mask,gnn_mask=gnn_mask)
...
class LM_QAGNN(nn.Module):
    ...
    def forward(self, *inputs, layer_id=-1, cache_output=False, detail=False):
        ...
        *lm_inputs, concept_ids, node_type_ids, node_scores, adj_lengths, edge_index, edge_type = _inputs
        edge_index, edge_type, edge_lengths = self.batch_graph(edge_index, edge_type, concept_ids.size(1))
        ...
        logits, attn = self.decoder(sent_vecs.to(node_type_ids.device),
                                    concept_ids,
                                    node_type_ids, node_scores, adj_lengths, adj, edge_lengths,
    ...
    def batch_graph(self, edge_index_init, edge_type_init, n_nodes):
        ...
        return edge_index, edge_type, edge_lengths
